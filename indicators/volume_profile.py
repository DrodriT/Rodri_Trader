...
Responsabilidad: calcular un Volume Profile aproximado sobre una ventana
reciente de velas, devolviendo el Point of Control (POC) y el Value Area
(VAH/VAL).

Qué hace:
    - Reparte el volumen de cada vela entre los bins de precio que cubre
      su rango high-low.
    - Devuelve el bin con más volumen (POC) y expande hacia los lados
      hasta acumular ~70% del volumen total (Value Area), centrado en el
      POC.

Qué NO hace:
    - No añade columnas al DataFrame: se calcula bajo demanda sobre la
      ventana más reciente, ya que recalcularlo vela a vela sería carísimo
      y solo se necesita para la última vela cerrada
      (strategies/vp_mean_revert.py).

Migrado 1:1 desde indicators_rodri.py (add_volume_profile). Trabaja con
...
def add_volume_profile(df: pd.DataFrame, lookback: int = 100, bins: int = 24):
    """
    Volume Profile aproximado sobre las últimas 'lookback' velas.
    Devuelve un dict {"poc", "vah", "val"} o None si no hay rango válido.
    """
