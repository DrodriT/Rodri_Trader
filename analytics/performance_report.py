...
Responsabilidad: analizar el trade_log persistido (storage/state_store.py)
y desglosar el rendimiento (nº de operaciones, win rate, R medio) por
estrategia y por nivel de confluencia, para saber qué estrategias están
aportando y cuáles conviene ajustar o retirar.

Qué hace:
    - load_trade_log: lee el trade_log de un archivo de estado JSON.
    - analyze: agrupa resultados por estrategia individual (una operación
      cuenta para TODAS las estrategias que participaron en ella, no solo
      la primera) y por nivel de confluencia.
    - main: CLI para correr el análisis desde línea de comandos.

Qué NO hace:
    - No modifica el estado ni el trade_log (es de solo lectura).
    - No se ejecuta como parte del bucle en vivo del bot (engine/); es un
      proceso aparte, bajo demanda.

Uso:
    python3 -m analytics.performance_report                  # usa cfg.STATE_FILE
    python3 -m analytics.performance_report otro_estado.json # o un archivo concreto

Migrado 1:1 desde analyze_rodri.py (load_trade_log, analyze, fmt_row, main).
"""
import json
import sys
...
def analyze(trade_log: list[dict]):
...
def main():
    path = sys.argv[1] if len(sys.argv) > 1 else config.STATE_FILE
